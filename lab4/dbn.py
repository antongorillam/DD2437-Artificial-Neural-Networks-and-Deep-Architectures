...
class DeepBeliefNet():    
    ...
    def recognize(self,true_img,true_lbl):
        ...
        prob_hid, hid_out = self.rbm_stack["vis--hid"].get_h_given_v_dir(vis)
        prob_pen, pen_out = self.rbm_stack["hid--pen"].get_h_given_v_dir(prob_hid)

        top_v = np.concatenate((prob_pen, lbl), axis=1)

        for i in range(self.n_gibbs_recog):
            print(f'running Gibbs recognize iteration: {i}/{self.n_gibbs_recog}')
            top_v[:,:-n_labels] = prob_pen
    ...
    def generate(self,true_lbl,name):
        ...
        n_labels = top.n_labels

        prob_top_v = np.random.uniform(0,1,(lbl.shape[0], top.bias_v.shape[0]))
        top_v = sample_binary(prob_top_v)

        for _ in range(self.n_gibbs_gener):

            prob_top_v[:,-n_labels:] = lbl
            top_v[:,-n_labels:] = lbl

            prob_top_h, top_h = top.get_h_given_v(prob_top_v)
            prob_top_v, top_v = top.get_v_given_h(prob_top_h)
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try :
            ...
            """        
            recon_losses1 = self.rbm_stack['vis--hid'].cd1(visible_trainset=vis_trainset, n_iterations=n_iterations)
            self.savetofile_rbm(loc="trained_rbm",name="vis--hid")
            df = pd.DataFrame(recon_losses1, columns=['num_iterations', 'recon_losses'])
            n_hidden1 = self.rbm_stack['vis--hid'].ndim_hidden
            df.to_csv(f'recon_losses1_nHidden_{n_hidden1}_dbn.csv')
            ...
            """            
            self.rbm_stack["vis--hid"].untwine_weights()
            _, hid_out = self.rbm_stack["vis--hid"].get_h_given_v_dir(vis_trainset)
            recon_losses2 = self.rbm_stack['hid--pen'].cd1(visible_trainset=hid_out, n_iterations=n_iterations)            
            self.savetofile_rbm(loc="trained_rbm",name="hid--pen") 
            df = pd.DataFrame(recon_losses2, columns=['num_iterations', 'recon_losses'])
            n_hidden2 = self.rbm_stack['hid--pen'].ndim_hidden
            df.to_csv(f'recon_losses2_nHidden_{n_hidden2}_dbn.csv')           
            ...
            self.rbm_stack['pen+lbl--top'].cd1(visible_trainset=pen_out, n_iterations=n_iterations)
