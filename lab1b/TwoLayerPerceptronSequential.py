...
import numpy as np
class TwoLayerPerceptronSequential:
    ...
    def forwardPass(self, w, xPair):
        print(w.shape)
        print(x.shape)
        w = w.reshape(1, w.size)
        x = x.reshape(1, x.size)
        # get xinput correct size
        hIn = w @ x
        print(hIn)
        # get hin
    ...
    def train(self, epochs, validation=[]):
        ...
        for epoch in range(epochs):
            ...
            print('xInput.T: ',self.xInput.T.shape)
            for w, x in zip(self.W, self.xInput.T):
                hIn, Oin, oOut, hOut = self.forwardPass(w, x)
                None
    ...
    #     for epoch in range(epochs):
    #         for w, x in zip(self.W, self.xInput):
    #             hIn, Oin, oOut, hOut = self.forwardPass(w, x)
    #             None
